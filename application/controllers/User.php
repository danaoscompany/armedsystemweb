...
FROM videos HAVING distance < 25 ORDER BY distance;')->result_array();
	...
	public function get_kemampuan_perorangan() {
	...
	public function get_pelanggaran() {
		$pelanggaran = $this->db->get("pelanggaran")->result_array();
		echo json_encode($pelanggaran);
	}
	
	public function get_menarmed() {
		$categories = $this->db->get("menarmed_categories")->result_array();
		for ($i=0; $i<sizeof($categories); $i++) {
			$categories[$i]['menarmed'] = $this->db->query("SELECT * FROM `menarmed` WHERE `menarmed_category_id`=".$categories[$i]['id'])->row_array();
		}
		echo json_encode($categories);
	}
	
	public function get_pelanggaran_by_menarmed_id() {
		$menarmedID = intval($this->input->post('menarmed_id'));
		$pelanggaran = $this->db->query("SELECT * FROM `pelanggaran` WHERE `menarmed_id`=".$menarmedID)->result_array();
		echo json_encode($pelanggaran);
	}
}
